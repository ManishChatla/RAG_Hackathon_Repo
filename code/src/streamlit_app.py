...
@st.cache_resource
def load_rag_store():
    return ChromaRAG(db_path=Config.CHROMA_DB, collection_name=Config.collection_name)

# def generate_response(query_input, rag_estore):
#     start_time = time.time()
#     retrieved_docs, full_results = rag_estore.retrieve(query_input, top_k=10)
#
#     # Retrieval confidence
#     confidences = []
#     if full_results and "distances" in full_results:
#         distances = full_results["distances"][0]
#         max_dist = max(distances) if distances else 1
#         confidences = [round(1 - (d / max_dist), 2) for d in distances]
#     else:
#         confidences = [1.0] * len(retrieved_docs)
#
#     rag_prompt = make_rag_prompt(query=query_input, relevant_passage="\n".join(retrieved_docs))
#     answer_text = query_gemini(rag_prompt)
#     token_count = len(answer_text.split())
#     latency = round(time.time() - start_time, 2)
#     answer_confidence = round(sum(confidences) / len(confidences), 2) if confidences else 0.0
#
#     ids = list(
#         set([item['id'] for sublist in full_results.get('metadatas', []) for item in sublist])) if full_results else []
#
#     return {
#         "answer": answer_text,
#         "article_ids": ids,
...
def compute_confidences(full_results):
    ...
    # Distance-based score (closer -> higher confidence)
    dist_scores = 1 - (distances - d_min) / (d_max - d_min + 1e-9)

    # Rank-based decay (top results get higher confidence)
    rank_scores = np.exp(-0.3 * np.arange(len(distances)))

    # Spread consistency (tighter cluster => higher reliability)
    ...
    return np.round(final_conf, 2).tolist()
...
def generate_response(query_input, rag_estore):
    ...
    confidences = compute_confidences(full_results)
    ...
    token_count = len(answer_text.split())
    latency = round(time.time() - start_time, 2)
    answer_confidence = round(np.mean(confidences), 2) if confidences else 0.0
...
def generate_bulk_answers(df_ques, rag_estore):
    output = []
    for i, row in df_ques.iterrows():
        question = row["question"]
        res = generate_response(question, rag_estore)
        output.append({
            "question_id": i + 1,
            "question": question,
            "answer": res["answer"],
...
.answer-card h3 { color: #2c3e50; margin-bottom: 10px; }
...
with tab1:
    ...
    if st.button("Get Answer", use_container_width=True):
        if user_query.strip():
            ...
            result = generate_response(user_query, rag_store)
            loader_text.empty()
    ...
    if uploaded_file:
        ...
        if "question" not in df.columns:
            ...
            if st.button("Generate Answers for All", use_container_width=True):
                ...
                # Generate answers
                results = generate_bulk_answers(df, rag_store)
                loader_text.empty()
